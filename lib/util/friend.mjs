...
export const compareFriend = (prospective, existing) => {
  const {
    id: idA,
    friend: { displayName: displayNameA, externalAuths: authA },
  } = prospective;
  const {
    id: idB,
    friend: { displayName: displayNameB, externalAuths: authB },
  } = existing;
...
/**
 * Determine if there is already a friendship.
 *
...
export const findFriendsToAdd = (friendsToAdd, searchScope) => {
  ...
  friendsToAdd.forEach((friendToAdd) => {
    let { friend } = friendToAdd;
    const isFriend = isExistingFriend(friendToAdd, searchScope);
    let status = "MISSING";

    if (isFriend) {
      friend = isFriend.friend;
      status = friend.direction ? `PENDING_${friend.direction}` : "CURRENT";
    }
